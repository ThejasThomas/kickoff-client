...
import type { ExtendableItem, TableAction, TableColumn, TableFilter } from "@/types/table_type"
...
export default function OwnerManagement() {
  ...
  const fetchOwners = async (params: FetchParams): Promise<ApiResponse<OwnerData>> => {
    try {
      ...
      const turfOwners: OwnerData[] = (response.users as ITurfOwner[]).map((user) => ({
        _id: user._id,
  ...
  const changeOwnerStatus = async (ownerId: string, newStatus: OwnerStatus) => {
    try {
      const response = await adminService.updateEntityStatus("turfOwner", ownerId, newStatus)
      if (response.success) {
