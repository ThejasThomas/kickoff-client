...
import {type  PaginationInfo, type BaseItem, type TableAction, type TableConfiguration } from "@/types/table_type";
...
const GenericTableInner = <T extends BaseItem>({
    ...
    onSecondClick,
    secondButtonIcon
}: GenericTableProps<T>, ref: React.Ref<TableRef<T>>) => {
    ...
    useImperativeHandle(ref, () => ({
        updateItemOptimistically: (id: string, updates: Partial<T>) => {
            setData(prevData => prevData.map(item => item._id === id ? { ...item, ...updates } : item))
        },
